class ProfessersController < ApplicationController
  ...
  def show
    ...
    @professer ||= Professer.find_by(params[:id])
    #redirect_to professer_path(@professer)
  end
  ...
  def create
    ...
    @professer.Email.downcase!
    respond_to do |format|
      if @professer.save
        format.html { redirect_to '/', notice: 'Professer was successfully created.' }
  ...
  private
    ...
    def professer_params
      params.require(:professer).permit(:Email, :Name, :Password, :Section_num, :Course_num, :Teaching_year, :Title, :password_digest)
    end
end
